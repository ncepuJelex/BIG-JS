$(document).ready(function() {
	...
	document.getElementById('task').onchange = maintenanceEdit.taskChange;
...
$('#ybtbatchForm').bootstrapValidator({
	...
	alert(123);
	console.log(bank,task,taskDate,clazz);
...
var maintenanceEdit = {
	...
	taskChange : function() {
		var taskVal = $('#ybtbatchForm select#task').find('option:selected').val();
		var downloadTasks = ['policy','fApply','BQApply','invalidBank','fresultBank','sresultBank'];
		if($.inArray(taskVal, downloadTasks) > -1) {
			$("#ybtbatchForm #download").prop("disabled",false);
			$("#ybtbatchForm #downloadcheck").prop("disabled",false);
			$('#ybtbatchForm').bootstrapValidator('updateStatus', "inlineRadioOptions", 'NOT_VALIDATED');
		} else {
